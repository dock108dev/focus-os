...
TECH_SYMBOLS = {"AAPL", "MSFT", "NVDA", "GOOGL", "GOOG", "AMZN", "META", "TSLA", "AMD", "AVGO"}
...
def is_technology(holding: Holding) -> bool:
    return holding.symbol.upper() in TECH_SYMBOLS or "tech" in holding.asset_class.lower()
...
def summarize(holdings: Iterable[Holding]) -> dict:
    ...
    cash_available = sum((holding_value(row) for row in rows if is_cash(row)), Decimal("0"))
    ...
    allocation_rows = [
        ...
        for label, value in sorted(allocation.items(), key=lambda item: item[1], reverse=True)
    ]
    ...
    return {
        ...
        "cash_percent": float(cash_available / current_value * 100) if current_value else 0,
...
def build_attention(holdings: Iterable[Holding], summary: dict) -> list[dict]:
    ...
    if total and cash >= Decimal(str(MIKE_PROFILE["cash_attention_min"])) and cash_percent >= Decimal(
        str(MIKE_PROFILE["cash_attention_pct"])
    ):
        items.append(
            AttentionItem(
                ...
                classification=(
                    "action_required"
                    if cash_percent >= Decimal(str(MIKE_PROFILE["cash_attention_pct"])) * Decimal("1.4")
                    else "awareness"
                ),
            )
        )

    tech_value = sum((holding_value(row) for row in rows if is_technology(row)), Decimal("0"))
    ...
    for row in rows:
        ...
        if concentration >= Decimal(str(MIKE_PROFILE["single_position_concentration_pct"])):
            items.append(
                AttentionItem(
    ...
    return [item.__dict__ for item in ranked[:10]]


def build_opportunities(holdings: Iterable[Holding], summary: dict, attention: list[dict]) -> list[dict]:
    rows = list(holdings)
    ...
    underwater = [
        row
        for row in rows
        if row.cost_basis and Decimal(row.cost_basis) > holding_value(row) and not is_cash(row)
    ]
    for row in sorted(
        underwater,
    ...
    return [item.__dict__ for item in sorted(opportunities, key=lambda item: item.priority, reverse=True)[:5]]
...
def build_recommended_actions(
    ...
    def add(title: str, reason: str, priority: int, detail_id: str) -> None:
        if title in seen:
            return
        seen.add(title)
        actions.append({"title": title, "reason": reason, "priority": priority, "detail_id": detail_id})

    for item in opportunities:
        if item["title"] == "High cash position":
            add("Deployable cash is elevated.", item["action"], item["priority"], "finance:cash")
        elif "review candidate" in item["title"].lower():
            symbol = item["title"].split()[0]
            add(f"{symbol} is in a pullback range.", item["action"], item["priority"], f"finance:position:{symbol}:pullback")
        elif item["title"] == "Rebalance candidate":
            add("Check allocation drift.", item["action"], item["priority"], "finance:technology")

    for item in attention:
        if "Technology allocation" in item["title"]:
            add("Avoid adding more technology by default.", item["action"], item["priority"], item.get("detail_id", "finance:technology"))
        elif "cash is available" in item["title"]:
            add("Cash purpose is unresolved.", item["action"], item["priority"], item.get("detail_id", "finance:cash"))
    ...
    for briefing in topic_briefings:
        ...
        if topic == "Golf":
            ...
            add("Yankees highlights may be worth catching.", action, priority, detail_id)
        elif topic:
...
def build_portfolio_status_item(financial_attention: Iterable[dict]) -> dict:
    pullback_item = next(
        (item for item in financial_attention if "from cost basis" in item["title"].lower()),
        None,
    )
    if pullback_item:
...
def build_morning_attention_feed(attention_groups: Iterable[Iterable[dict]], financial_attention: Iterable[dict]) -> list[dict]:
    candidates = sorted(
        [item for group in attention_groups for item in group],
    ...
    portfolio_status = build_portfolio_status_item(financial_attention)
